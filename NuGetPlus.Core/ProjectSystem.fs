...
type ProjectSystem (projectFile : string) =
    ...
    let GetReference path =
            ...
            |> Seq.filter (fun i -> i.EvaluatedInclude.StartsWith(name, StringComparison.OrdinalIgnoreCase))
            |> Seq.tryFind (fun i -> AssemblyName(i.EvaluatedInclude).Name.Equals(name, StringComparison.OrdinalIgnoreCase))
    interface IProjectSystem with
        member x.TargetFramework
            with get () = framework
        member x.ProjectName
            with get () = projectName
        member x.AddReference (path, stream) =
            let fullPath = PathUtility.GetAbsolutePath(x.Root, path)
            let relPath = PathUtility.GetRelativePath(project.FullPath, fullPath)
        ...
        member x.RemoveReference name =
            ...
            | Some i ->
                ...
                project.Save()
            | None -> ()
        member x.IsSupportedFile path =
            true
        member x.ResolvePath path =
            path
        member x.IsBindingRedirectSupported
            with get () = true
        member x.AddImport ((targetPath : string), location) =
            if project.Xml.Imports = null 
                || project.Xml.Imports |> Seq.forall (fun import -> not <| targetPath.Equals(import.Project, StringComparison.OrdinalIgnoreCase)) then
                    project.Xml.AddImport(targetPath) |> ignore
                    project.ReevaluateIfNecessary ()
                    project.Save ()
        member x.RemoveImport (targetPath : string) =
            match project.Xml.Imports |> Seq.tryFind (fun import -> targetPath.Equals(import.Project, StringComparison.OrdinalIgnoreCase)) with
            | None -> ()
            | Some i -> 
        ...
        member x.FileExistsInProject (path : string) =
            project.Items
            |> Seq.exists (fun i -> i.EvaluatedInclude.Equals(path, StringComparison.OrdinalIgnoreCase) && (String.IsNullOrEmpty(i.ItemType) || i.ItemType.[0] <> '_'))
            
    interface IPropertyProvider with
        member x.GetPropertyValue name =
            project.GetPropertyValue(name) :> obj
