...
type XmlTransformer() = 
    let (configMappings : IDictionary<XName, System.Action<XElement, XElement>>) = 
        dict 
            [(XName.Get "configSections", 
              
              Action<XElement, XElement>
                  (fun parent element -> parent.AddFirst(element)))]
        ...
        member x.TransformFile(file, targetPath, projectSystem) = 
            ...
            let doc = 
                ...
                | false -> 
                    ...
                    projectSystem.AddFile
                        (targetPath, fun (stream : Stream) -> xDoc.Save(stream))
                    xDoc
            doc.Root.MergeWith(xml, configMappings) |> ignore
            projectSystem.AddFile
                (targetPath, fun (stream : Stream) -> doc.Save(stream))
        
        member x.RevertFile(file, targetPath, matchingFiles, projectSystem) = 
            raise 
            <| NotImplementedException
                   ("RevertFile for XmlTransformer not implemented")